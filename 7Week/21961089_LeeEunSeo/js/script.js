...
// classTitle.innerText = "안녕하세요."; // 세 개의 class에 전체 적용 불가.
...
console.log("row class의 개수는 " + rowCount + "개 입니다."); // row class의 개수는 4개 입니다.
...
var classRow = idArticle.getElementsByClassName("row");
classRow[0].classList.add("red");
// classRow[2].classList.remove("row"); // 세 번째 자식 요소인 section의 class만 삭제됨.
idArticle.removeChild(classRow[2]); // 세 번째 자식 요소인 section이 삭제됨.
...
// Answer 9.
function enterName() {
    var isComplete = prompt("이름을 입력해주세요.");
    if (isComplete === null) { // 조건을 !isComplete 로 설정할 경우, 문자를 입력하지 않고 확인을 클릭할 때마다 console에 "입력이 취소되었습니다."가 출력됨.

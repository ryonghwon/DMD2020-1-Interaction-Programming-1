...
*/
// Answer 4.
var input_text = document.getElementsByClassName("title"); // 변수 input_text 생성 후 클래스 이름이 title 인것만 반환하여 대입

for(var i = 0; i < input_text.length; i ++){ // for문을 사용하여 클래스 이름이  title인것만큼 반복
    input_text[i].innerText = '안녕하세요.'; // 안녕하세요를 대입
    input_text[i].style.color = "blue"; // 색깔을 파란색으로 설정
}
...
{
    var row_amount = document.getElementsByClassName("row");
    return row_amount.length;     
}
...
var hello1s = setInterval(function(){
    console.log('hello' + count);
    if(count == 10){
        clearInterval(hello1s);
    }
count++}, 1000);
...
var row1 = article1.getElementsByClassName("row");
article1.removeChild(row1[2]);

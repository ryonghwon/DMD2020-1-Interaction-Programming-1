...
function compareNumbers(a, b) {
    if (typeof b === "undefined") {
        console.log("두 번째 매개 변수의 값을 찾을 수 없습니다.");
    }
    else if (a >= b) {
        console.log("첫 번째 매개 변수에 할당된 전달 인자의 값이 두 번째 매개 변수에 할당된 전달 인자의 값보다 크거나 같습니다.");
    }
    else {
        console.log("첫 번째 매개 변수에 할당된 전달 인자의 값이 두 번째 매개 변수에 할당된 전달 인자의 값보다 작습니다.");
    }
}

compareNumbers(10); // "두 번째 매개 변수의 값을 찾을 수 없습니다."

compareNumbers(20, 10); // "첫 번째 매개 변수에 할당된 전달 인자의 값이 두 번째 매개 변수에 할당된 전달 인자의 값보다 크거나 같습니다."

compareNumbers(10, 20); // "첫 번째 매개 변수에 할당된 전달 인자의 값이 두 번째 매개 변수에 할당된 전달 인자의 값보다 작습니다."
...
var results = getTotal(5, "add");
console.log(results); // 15
var results = getTotal(10, "add");
console.log(results); // 55

var results = getTotal(5, "multiply");
console.log(results); // 120
var results = getTotal(10, "multiply");
console.log(results); // 3628800

var results = getTotal(100);
...
function getMaxValue(arr) {
    var max = arr[0];
    var order = 0;
    if (typeof arr[0] === "number") { // 배열의 데이터 타입이 숫자일 경우 실행.
...
var maxValue = getMaxValue([10, -4, 7, 100, "hello", -50]);
console.log(maxValue); // 3

var maxValue = getMaxValue([-400, "world", 60, 0, {}, 1000]);

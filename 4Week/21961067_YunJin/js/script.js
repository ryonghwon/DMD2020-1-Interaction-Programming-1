...
function getMaxValue(z){
    var max = z[0];
    // rh :  이 부분이 잘 못 되었습니다.
    // 처음 초기값이 무조건 0 번째로 들어가는데 이렇게 될 경우, _9 처럼 전달인자가 들어올 때 0 번째가 숫자가 아닌 경우가 생깁니다.
    // max 는 처음에 null 로 숫자가 없는 상태로 준비하고, 반복문이 돌면서 숫자 타입이 되는 시점에 초기값을 그 숫자로 맞춰주는 한번의 조건이 더 필요할 것 같습니다.
    // 이 부분을 처리하는 로직을 구성하기는 좀 어려울 것 같은데요. 우선 도전해보세요~
    var num = 0;
...
// 반복문 안에  if(typeof z[i] === "number") 를 추가해보았는데, 똑같이 0이 출력되네요,, 잘 모르겠습니다,,!
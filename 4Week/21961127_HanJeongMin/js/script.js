...
2) 조건1 : 1(순서) 함수가 호출될 때, 두번째 매개변수의 전달인자 값이 할당되지 않은 경우, 
아래와 같이 console 에 출력되도록 작성합니다.
(두번째 매개변수의 값을 찾을 수 없습니다.)

3) 값이 할당된 경우 4(순서)에 이어서 작성합니다.
4) 조건2 : 첫번째 매개변수의 전달인자 값과 두번째 매개변수의 전달인자 값을 비교하여 크거나 같은 경우, 
아래와 같이 console 에 출력되도록 작성합니다.
...
2) 조건 1-1 : 1(순서) 함수가 호출될 때, 두번째 매개변수의 전달인자 값이 "add" 인 경우, 
...
var sum3 = getTotal(5, "multiply");
console.log(sum3);

var sum4 = getTotal(10, "multiply");
console.log(sum4);
var sum5 = getTotal(100);
console.log(sum5);
...
function getMaxValue(a1)
{
var max = a1[0];
var num = 0;  
for(var g = 1; g < a1.length; g++ )
{
    if(max < a1[g])
        (max = a1[g]);
        num = g;
...
var name;
name = getMaxValue([10, -4, 7, 100, "hello", -50]);
console.log(name -2);
name = getMaxValue([-400, "world", 60, 0, { }, 1000]);
console.log( name);

...
//원소값들의 순서를 뒤바꾸어 주는 명령이 필요함.
cars.reverse(); //reverse를 이용해 원소값을 거꾸로 정렬.
console.log(cars);
...
var numbers; //변수 선언.
numbers = [-1, 2, 5, 10, 1, -10, 8, 4];

console.log(Math.max.apply(null, numbers)); //Math.max 를 이용해 원소값 중 큰 숫자를 찾았다. 출력 10.
console.log(Math.min.apply(null, numbers)); //Math.min 을 이용해 원소값 중 작은 숫자를 찾았다. 출력 -10.
...
//getStar 함수 선언, num1 이라는 매개변수를 지정했다.
function getStar (num1){
    if ( typeof num1 !== "number"){ //num1의 데이터 타입이 숫자가 아닐 때의 조건문
        console.log("숫자만 입력 가능합니다.");
        return false; //false를 출력하기 위해 false를 반환한다.
    }
    if (typeof num1 === "number"){ //num1의 데이터 타입이 숫자일 때의 조건문
        var a = "";
...
var fruit;
fruit = ['Apple', 'Orange', 'Grape', 'Melon']; //fruit 변수 선언, 배열 할당.

fruit.unshift("Mango"); //unshift 사용해 맨 첫 번째에 요소를 추가함.
console.log(fruit); //["Mango", "Apple", "Orange", "Grape", "Melon"]

fruit.splice(3, 0, "Cherry"); //splice 사용. 3번째 문자열부터, 뒤의 문자열은 0개를 삭제하고 수정함.
console.log(fruit); //["Mango", "Apple", "Orange", "Cherry", "Grape", "Melon"]

fruit.reverse(); //reverse 사용. 배열을 거꾸로 뒤집음.

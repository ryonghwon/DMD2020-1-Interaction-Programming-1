var accordionEl = document.querySelector("#accordion"),
    ...
    viewItemEls = accordionEl.querySelectorAll(".view-item"),
...
function onClickViewItem(e){
    ...
    if(!el.classList.contains("selected")){
        // selected 클래스가 가지고 있지 않다면 활성화가 가능한 요소들.
        _cuId = index;
        if(_exId !== null){
            // 처음부터 열려있는 아코디언 메뉴가 없기 때문에 _exId 의 값은 처음에 null 이다.
            // 따라서 null 이 아닌 경우부터 이전 항목에 대한 체크가 가능하다.
...
function onClickClose(e){
    ...
    // ****중요*****
    e.stopPropagation();
    // 이벤트 버블링이 일어나면 아코디언 메뉴가 비활성화되었다가(close), 부모요소인 view-item의 이벤트가
    // 한번 더 발생되기 때문에 해당 메뉴가 다시 활성화된다.(view-item )

...
var spNum = document.querySelector("span#student-number");

btnNum.addEventListener("click", onClickbtnNum);

function onClickbtnNum(event) {
    event.preventDefault(); 
    spNum.innerText = "21861122";
    alert("학번이 입력되었습니다.");
}


var btnNa = document.querySelector("button#btn-student-name");
var spNa = document.querySelector("span#student-name");

btnNa.addEventListener("click", onClickbtnNa);

function onClickbtnNa(event) {
    event.preventDefault();
    spNa.innerText = "최성혜";
    alert("이름이 입력되었습니다.");
}
...
*/
// Answer 2.
var btnWinS = document.querySelector("button#btn-window-size");

winWidth = window.innerWidth;
winHeight = window.innerHeight;

btnWinS.addEventListener("click", onClickbtnWins);

function onClickbtnWins(event) {
    event.preventDefault();
...
*/
// Answer 3.
var btnChId = document.querySelector("button#btn-check-id");

btnChId.addEventListener("click", onClickbtnChId);

function onClickbtnChId(event) {
    event.preventDefault();
    var userId = document.getElementById("user_id");
    console.log("input#user_id - value 속성 값의 문자열 개수는 " + userId.value.length);
}
...
btnCl.addEventListener("click", onClickbtnCl);

function onClickbtnCl(event) {
    event.preventDefault();
    location.reload();
}
...
*/
// Answer 5.
var btnAl = document.querySelector("button#btn-apply");

btnAl.addEventListener("click", onClickbtnAl);

function onClickbtnAl(event) {
    event.preventDefault();
    var alFin = window.confirm("가입하시겠습니까?");
    if(alFin) {
        alert("가입을 환영합니다!");
    }
    if(!alFin) {
...
*/
// Answer 6.
var btnRt = document.querySelector("button#btn-reset");

btnRt.addEventListener("click", onClickbtnRt);

var inUsId = document.querySelector("input#user_id");
var inUsPw = document.querySelector("input#user_pass");

function onClickbtnRt(event) {
    event.preventDefault();
    var enterHun = prompt("초기화하시려면 100을 입력하세요.");
    if (enterHun == 100) {
        inUsId.value = "";
        inUsPw.value = "";
    }
...
function onClickbtnOdd(event) {
    event.preventDefault();
    var enterNum = prompt("숫자를 입력해주세요.");
...
*/
// Answer 8.
var btnAddCt = document.querySelector("button#btn-add-city");
btnAddCt.addEventListener("click", onClickbtnAddCt);
var ctUl = document.querySelector("ul#city-list");

function onClickbtnAddCt(event) {
    event.preventDefault();
    var enterCt = prompt("도시명을 입력하세요.");
    var newCt = document.createElement("li");
    newCt.innerText = enterCt;
    // console.log(newCt);
    ctUl.appendChild(newCt);
}
...
var pEls = document.getElementsByClassName("sentence");
var count = 0;

function onClickbtnColor(event) {
    event.preventDefault();
    for(var i = 0; i < pEls.length; i++) {
        pEls[i].style.color = "#f00";
    }
    pEls[count].style.color = "#000";
    count++;
    if (count == pEls.length) {
        count = 0;
    }   
}
...
btnGoT.addEventListener("click", onClickbtnGoT);

function onClickbtnGoT(event) {
    event.preventDefault();
    window.scrollTo(0, 0);
}